...
public class DataHubUsageEventsProcessor {
   ...
   */
  public void consumeKafka(final List<ConsumerRecord<String, String>> consumerRecords) {
  ...
  /** pgQueue transport: same indexing path as {@link #consumeKafka(List)}. */
  public void consumePgQueue(String logicalTopic, final List<QueueReceivedMessage> messages) {
    systemOperationContext.withSpan(
        "consume",
        () -> {
          List<DataHubUsageEventIndexer.IndexableUsageEvent> events =
          ...
          for (QueueReceivedMessage msg : messages) {
            ...
            final String record = PgQueueStringDecode.decodeAsUtf8(msg);
            log.debug(
                "Got DUE event key: {}, topic: {}, partition: {}, offset: {}, value size: {}, timestamp: {}",
                msg.routingKey(),
                logicalTopic,
            ...
            String documentId =
                generateDocumentId(eventDocument.get().getId(), msg.handle().enqueueSeq());
            events.add(
                new DataHubUsageEventIndexer.IndexableUsageEvent(eventDocument.get(), documentId));
          }
          usageEventIndexer.indexBatch(events);
