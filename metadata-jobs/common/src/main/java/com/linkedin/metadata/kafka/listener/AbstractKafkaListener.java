...
public abstract class AbstractKafkaListener<E, H extends EventHook<E>, R>
  ...
  public void consume(@Nonnull final ConsumerRecord<String, R> consumerRecord) {
    processEnvelope(InboundMetadataEnvelope.fromKafka(consumerRecord, consumerGroupId));
  }

  /** pgQueue and other transports call this after building {@link InboundMetadataEnvelope}. */
  @Override
  public void acceptInbound(@Nonnull final InboundMetadataEnvelope<R> envelope) {
    processEnvelope(envelope);
  }

  /** Shared path for Kafka and future non-Kafka transports. */
  protected void processEnvelope(@Nonnull final InboundMetadataEnvelope<R> envelope) {
    try {
