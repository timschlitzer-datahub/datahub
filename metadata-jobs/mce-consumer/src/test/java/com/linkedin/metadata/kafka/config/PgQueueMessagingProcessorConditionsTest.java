...
public class PgQueueMessagingProcessorConditionsTest {
  ...
  @Test
  public void cdcCondition_noMatchUnderPgQueueBecauseCdcRequiresKafka() {
    when(env.getProperty("mclProcessing.cdcSource.enabled", "false")).thenReturn("true");
    when(env.getProperty("MCE_CONSUMER_ENABLED")).thenReturn("true");

    assertFalse(new PgQueueMessagingAndCdcProcessorCondition().matches(context, metadata));
  }

  @Test
  public void cdcCondition_noMatchWhenKafkaTransport() {
    when(env.getProperty(MessagingTransport.PROPERTY, MessagingTransport.KAFKA))
  ...
  @Test
  public void mceCondition_matchesWhenPgQueueAndMceEnabled() {
    when(env.getProperty("MCE_CONSUMER_ENABLED")).thenReturn("true");

    assertTrue(
        new PgQueueMessagingAndMetadataChangeEventsProcessorCondition().matches(context, metadata));
  }
