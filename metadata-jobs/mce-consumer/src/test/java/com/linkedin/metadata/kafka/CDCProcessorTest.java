...
public class CDCProcessorTest {
  ...
  @Test
  public void testConsumePgQueueWithValidRecord() throws Exception {
    cdcProcessor.cdcMclProcessingEnabled = true;

    JsonNode mockCdcRecord = mock(JsonNode.class);
    when(mockObjectMapper.readTree("test-record")).thenReturn(mockCdcRecord);

    cdcProcessor.consumePgQueue(
        "test-topic", "test-key", "test-record", 0, 1L, 100, System.currentTimeMillis(), 0);
  ...
  @Test
  public void testConsumePgQueueWithNullValue() throws Exception {
    cdcProcessor.cdcMclProcessingEnabled = true;

    cdcProcessor.consumePgQueue(
        "test-topic", "test-key", null, 0, 1L, 0, System.currentTimeMillis(), 0);
  ...
  @Test
  public void testConsumePgQueueHandlesException() throws Exception {
    cdcProcessor.cdcMclProcessingEnabled = true;
