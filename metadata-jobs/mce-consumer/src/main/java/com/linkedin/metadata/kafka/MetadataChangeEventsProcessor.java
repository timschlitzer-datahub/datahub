...
public class MetadataChangeEventsProcessor {
  ...
  /** Used by {@link MetadataChangeEventsKafkaListener}. */
  @Deprecated
  public void consumeKafka(final ConsumerRecord<String, GenericRecord> consumerRecord) {
    systemOperationContext.withSpan(
        "consume",
        () -> {
          consumePgQueue(InboundMetadataEnvelope.fromKafka(consumerRecord, mceConsumerGroupId));
        },
  ...
  public void consumePgQueue(@Nonnull InboundMetadataEnvelope<GenericRecord> envelope) {
    systemOperationContext.withSpan(
        "consume",
        () -> {
