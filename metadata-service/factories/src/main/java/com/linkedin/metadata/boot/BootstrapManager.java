 ...
 *
 * <p>Async steps run on a fixed pool sized by {@code bootstrap.async.workerThreads} (default 5).
 * Each worker uses its own pooled JDBC connection and transactions; lowering the thread count
 * trades bootstrap throughput for less concurrent pressure on the metadata DB pool if needed.
 */
@Slf4j
@Component
public class BootstrapManager {
  ...
  public BootstrapManager(
      final List<BootstrapStep> bootSteps,
